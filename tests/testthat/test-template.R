...
#    `functionName` is a function's name in your module (e.g., `LandR-Biomass-FuelsEvent1`).
# 2. Copy this file to the tests folder (i.e., `E:/GitHub/LandR-Biomass-Fuels/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LandR-Biomass-Fuels")
  path <- list(modulePath = "E:/GitHub",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    LandR-Biomass-Fuels = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LandR-Biomass-FuelsEvent1", envir = .GlobalEnv)) {
    simOutput <- LandR-Biomass-FuelsEvent1(mySim)
  } else {
  ...
  if (exists("LandR-Biomass-FuelsEvent2", envir = .GlobalEnv)) {
    simOutput <- LandR-Biomass-FuelsEvent2(mySim)
  } else {
